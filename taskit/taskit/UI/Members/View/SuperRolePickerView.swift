...
class SuperRolePickerView: UIView {

    var superRoles = [SuperRole.owner, SuperRole.admin, SuperRole.standard]
    var selectedIndex = 0
    var selection: SuperRole? {
        didSet {
            selectedIndex = superRoles.index(of: selection ?? SuperRole.standard) ?? 0
        }
    }
    ...
    @objc func done() {
        self.removeFromSuperview()
        selectBlock?(superRoles[selectedIndex])
    }
}

extension SuperRolePickerView: UIPickerViewDataSource, UIPickerViewDelegate {
    func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return 1
    }
    
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return superRoles.count
    }
    
    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        return superRoles[row].descString
    }
